...
class DetailedFlagView(context: Context, layout: Int) : FlagView(context, layout) {
    ...
    override fun onRefresh(envelope: ColorEnvelope?) {
        ...
        flagColor.background.colorFilter =
        ...
        flagColorArgb.text = if (isAlphaDisabled) {
            String.format(
                context.getString(R.string.rgb_),
                envelope.color.red,
            ...
            String.format(
                context.getString(R.string.argb_),
                envelope.color.alpha,
                envelope.color.red,
                envelope.color.green,
                envelope.color.blue
            )
        ...
        flagColorHex.text = if (isAlphaDisabled)
            String.format(
                context.getString(R.string.hex_),
        ...
        flagColorHsv.text = String.format(
            context.getString(R.string.hsv_),
            String.format(Constants.HSV_FORMAT_STRING, hsv[0]),
            String.format(
                Constants.HSV_FORMAT_STRING, hsv[1]
            ),
