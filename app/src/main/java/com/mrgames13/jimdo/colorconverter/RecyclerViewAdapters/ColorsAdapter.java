...
public class ColorsAdapter extends RecyclerView.Adapter<ColorsAdapter.ViewHolderClass> {
    ...
    public ColorsAdapter(Resources res, ArrayList<Color> colors) {
        this.res = res;
        this.colors = colors;
    }
    ...
    public void onBindViewHolder(final ViewHolderClass holder, final int pos) {
        ...
        String details_string = "RGB: " + String.valueOf(color.getRed()) + ", " + String.valueOf(color.getGreen()) + ", " + String.valueOf(color.getBlue()) + " • HEX: #" + hex_red + hex_green + hex_blue + " • HSV: " + String.format("%.02f", hsv[0]) + ", " + String.format("%.02f", hsv[1]) + ", " + String.format("%.02f", hsv[2]);
